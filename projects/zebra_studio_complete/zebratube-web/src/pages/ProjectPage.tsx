...
export default function ProjectPage(){
  ...
  useEffect(() => { if (!id) return; api.getProject(id).then(setProject); api.getTasks().then(setTasks) }, [id])
  return <div className="page"><h1>{project?.title || "Project"}</h1><TabBar tabs={["Overview","Graph","Scripts","Submissions","Assembly","Alternatives","History"]} active={tab} setActive={setTab} />
  {tab==="Overview" && <div className="card">Zebrapedia article and stats placeholder.</div>}
  {tab==="Graph" && <GraphView />}
