...
def narrative(graph):
    return {"category":"narrative_film","scenes":[
        {
            "scene_id": e.get("id", e.get("label","scene")),
            "summary": e.get("label",""),
            "characters": e.get("participants",[]),
            "uncertain_details": []
...
def ambiguity(graph):
    return {"category":"ambiguity_diffusion","units":[
        {"label": a.get("label",""), "possibilities": a.get("possibilities",[]), "status": a.get("status","open")} for a in graph.get("ambiguities",[])
    ]}

BUILDERS = {
    "narrative_film": narrative,
    "diagrammatic_structure": diagram,
    "ambiguity_diffusion": ambiguity,
}
