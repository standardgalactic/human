...
def assign_dependencies(tasks):
    ...
    for i, t in enumerate(tasks):
        if i > 0:
            G.add_edge(tasks[i-1]["id"], t["id"])
            t["deps"].append(tasks[i-1]["id"])
    centrality = nx.betweenness_centrality(G, normalized=True) if len(G) > 1 else {tasks[0]["id"]:0.0} if tasks else {}
    for t in tasks:
        t["assembly_weight"] = round(0.5 + 1.5 * centrality.get(t["id"], 0.0), 3)

def write_task_package(task, out_dir: pathlib.Path):
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as z:
        for f in task_dir.iterdir():
            z.write(f, arcname=f"{task['id']}/{f.name}")
...
def main():
    ...
    for p in proj_dir.glob("*.json"):
        ...
        tasks.extend(projection_to_tasks(p.stem, data))
    assign_dependencies(tasks)
    for t in tasks:
        write_task_package(t, out_dir)
